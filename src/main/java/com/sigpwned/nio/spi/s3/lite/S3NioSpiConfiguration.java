...
import com.sigpwned.aws.sdk.lite.core.util.Pair;
...
public class S3NioSpiConfiguration extends HashMap<String, Object> {
   ...
   */
  public static final String S3_SPI_READ_MAX_FRAGMENT_SIZE_PROPERTY =
      "s3.spi.read.max-fragment-size";
  /**
   ...
   */
  public static final String S3_SPI_READ_MAX_FRAGMENT_NUMBER_PROPERTY =
      "s3.spi.read.max-fragment-number";
  /**
   * The default value of the maximum fragment size property
   */
  ...
  public S3NioSpiConfiguration(Map<String, ?> overrides) {
    ...
    // setup defaults
    //
    put(S3_SPI_READ_MAX_FRAGMENT_NUMBER_PROPERTY,
        String.valueOf(S3_SPI_READ_MAX_FRAGMENT_NUMBER_DEFAULT));
    put(S3_SPI_READ_MAX_FRAGMENT_SIZE_PROPERTY,
        String.valueOf(S3_SPI_READ_MAX_FRAGMENT_SIZE_DEFAULT));
  ...
  protected S3NioSpiConfiguration(Properties overrides) {
    Objects.requireNonNull(overrides);
    overrides.stringPropertyNames().forEach(key -> put(key, overrides.getProperty(key)));
  }
  ...
  public S3NioSpiConfiguration withEndpoint(String endpoint) {
    ...
    if (!endpoint.isEmpty() && !ENDPOINT_REGEXP.matcher(endpoint).matches()) {
      throw new IllegalArgumentException(String.format(
          "endpoint '%s' does not match format host:port where port is a number", endpoint));
    }
  ...
  /**
   * Fluently sets the value of accessKey and secretAccessKey given a {@code AwsCredentials} object.
   *
   ...
   * {@code S3_SPI_FORCE_PATH_STYLE_PROPERTY} to the map unless the given value is null. If null,
   * {@code S3_SPI_FORCE_PATH_STYLE_PROPERTY} is removed from the map.
   *
  ...
  public String getEndpointProtocol() {
    String protocol =
        (String) getOrDefault(S3_SPI_ENDPOINT_PROTOCOL_PROPERTY, S3_SPI_ENDPOINT_PROTOCOL_DEFAULT);
   ...
   * <p>
   * 1. {@code withCredentials(String accessKey, String secretAcccessKey)} 2.
   * {@code withCredentials(AwsCredentials credentials)}
   * <p>
   * The latter takes the priority, so if both are used, {@code getCredentials()} returns the most
   * complete object, which is the value of the property {@code S3_SPI_CREDENTIALS_PROPERTY}
   *
  ...
  public AwsCredentials getCredentials() {
    ...
    if (containsKey(AWS_ACCESS_KEY_PROPERTY)) {
      return AwsBasicCredentials.create((String) get(AWS_ACCESS_KEY_PROPERTY),
  ...
  /**
   * Generates an environment variable name from a property name. E.g 'some.property' becomes
   * 'SOME_PROPERTY'
   *
