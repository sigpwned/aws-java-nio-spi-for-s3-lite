...
public class S3Path implements Path {
  ...
  /**
   * Construct a Path in the parent FileSystem using the POSIX style. The path string is assumed to
   * follow the POSIX form with the "root" of the bucket being represented by "/". The supplied path
   * should not be a URI. It should not start with the string "s3:". For example, if this
   * S3FileSystem represents "{@code s3://my-bucket}" then "{@code s3://my-bucket/foo.txt}" should
   * be addressed by the path "/foo.txt" or by a path relative to the current working directory
   * following POSIX conventions. Further, although folders or directories don't technically exist
   * in S3 the presence of a directory is implicit if "{@code s3://my-bucket/someFolder/}" contains
   * objects and the Path to this folder is therefore valid.
   *
   * <p>
   * This library <b>DOES NOT</b> support S3 Paths that are not compliant with POSIX conventions.
   * For example, the URI {@code s3://my-bucket/../foo.txt} is legal in S3 but due to POSIX
   * conventions it will be unreachable through this API due to the special meaning of the ..
   * directory alias in POSIX.
   * </p>
   *
   * @param fsForBucket the filesystem for the bucket that holds this path
   * @param first the path string or initial part of the path string, may not be null. It may not be
   *        empty unless more is also null has zero length
  ...
  /**
   * Returns the name of the file or directory denoted by this path as a {@code Path} object. The
   ...
   * method does not eliminate special names such as "{@code .}" and "{@code ..}" that may be used
   * in some implementations. On UNIX for example, the parent of "{@code /a/b/c}" is "{@code /a/b}",
   * and the parent of {@code "x/y/.}" is "{@code x/y}". This method may be used with the
   * {@link #normalize normalize} method, to eliminate redundant names, for cases where
   * <em>shell-like</em> navigation is required.
   *
   * <p>
   * If this path has one or more elements, and no root component, then this method is equivalent to
   * evaluating the expression: <blockquote>
   *
   * <pre>
   ...
   *
   * @return the number of elements in the path, or {@code 0} if this path only represents a root
   *         component
   */
  @Override
   ...
   *
   * <p>
   * The {@code index} parameter is the index of the name element to return. The element that is
   * <em>closest</em> to the root in the directory hierarchy has the index {@code 0}. The element
   * that is <em>farthest</em> from the root has the index {@link #getNameCount count}{@code -1}.
   *
   * @param index the index of the element
   * @return the name element
   * @throws IllegalArgumentException if {@code index} is negative, {@code index} is greater than or
   *         equal to the number of elements, or this path has zero name elements
   */
  @Override
   ...
   *
   * <p>
   * The {@code beginIndex} and {@code endIndex} parameters specify the subsequence of name
   * elements. The name that is <em>closest</em> to the root in the directory hierarchy has the
   * index {@code 0}. The name that is <em>farthest</em> from the root has the index
   * {@link #getNameCount count}{@code -1}. The returned {@code Path} object has the name elements
   ...
   * @return a new {@code Path} object that is a subsequence of the name elements in this
   *         {@code Path}
   * @throws IllegalArgumentException if {@code beginIndex} is negative, or greater than or equal to
   *         the number of elements. If {@code endIndex} is less than or equal to
   *         {@code beginIndex}, or larger than the number of elements.
   */
  @Override
  public S3Path subpath(int beginIndex, int endIndex) {
    ...
    String path =
        String.join(PATH_SEPARATOR, pathRepresentation.elements().subList(beginIndex, endIndex));
   ...
   *
   * <p>
   * This path <em>starts</em> with the given path if this path's root component <em>starts</em>
   * with the root component of the given path, and this path starts with the same name elements as
   * the given path. If the given path has more name elements than this path then {@code false} is
   * returned.
   *
   * <p>
   * If this path does not have a root component and the given path has a root component then this
   * path does not start with the given path.
   *
   * <p>
   * If the given path is associated with a different {@code FileSystem} (s3 bucket) to this path
  ...
  public boolean startsWith(Path other) {
    return this.equals(other) || this.fileSystem.equals(other.getFileSystem())
        && this.isAbsolute() == other.isAbsolute() && this.getNameCount() >= other.getNameCount()
   ...
   * component of this path <em>ends with</em> the root component of the given path, and the
   * corresponding elements of both paths are equal. If the two paths are equal then they can be
   * said to end with each other. If this path does not have a root component and the given path has
   * a root component then this path does not end with the given path.
   *
   * <p>
  ...
  public boolean endsWith(Path other) {
    return this.equals(other) || this.fileSystem == other.getFileSystem()
        && this.getNameCount() >= other.getNameCount() && this
            .subpath(this.getNameCount() - other.getNameCount(), this.getNameCount()).equals(other);
  }

  /**
   * Tests if this path ends with a {@code Path}, constructed by converting the given path string,
   * in exactly the manner specified by the {@link #endsWith(Path) endsWith(Path)} method. On UNIX
   * for example, the path "{@code foo/bar}" ends with "{@code foo/bar}" and "{@code bar}". It does
   * not end with "{@code r}" or "{@code /bar}". Note that trailing separators are not taken into
   * account, and so invoking this method on the {@code
   * Path}"{@code foo/bar}" with the {@code String} "{@code bar/}" returns {@code true}.
   *
   * @param other the given path string
   * @return {@code true} if this path ends with the given path; otherwise {@code false}
   * @throws InvalidPathException If the path string cannot be converted to a Path.
  ...
  /**
   * Returns a path that is this path with redundant name elements eliminated. All occurrences of
   * "{@code .}" are considered redundant. If a "{@code ..}" is preceded by a non-"{@code ..}" name
   * then both names are considered redundant (the process to identify such names is repeated until
   ...
   *         empty path is returned if this path does have a root component and all name elements
   *         are redundant
   * @see #getParent
   * @see #toRealPath
   */
  ...
  public S3Path normalize() {
    ...
    final List<String> elements = pathRepresentation.elements();
    final Deque<String> realElements = new LinkedList<String>();
   ...
   *
   * <p>
   * If the {@code other} parameter is an {@link #isAbsolute() absolute} path then this method
   * trivially returns {@code other}. If {@code other} is an <i>empty path</i> then this method
   * trivially returns this path. Otherwise, this method considers this path to be a directory and
   * resolves the given path against this path by <em>joining</em> the given path to this path with
   * the addition of a separator ('/') and returns a resulting path that {@link #endsWith ends} with
   * the given (other) path.
   *
   * @param other the path to resolve against this path
   * @return the resulting path
   * @throws ProviderMismatchException if {@code other} is {@code null} or if it is not an instance
   *         of {@code S3Path}
  ...
  public S3Path resolve(Path other) {
    S3Path s3Other = requireNonNull(MorePaths.toS3Path(other));

    if (!this.bucketName().equals(s3Other.bucketName())) {
      throw new IllegalArgumentException(
          "S3Paths cannot be resolved when they are from different buckets");
    }
  ...
  /**
   * Converts a given path string to a {@code S3Path} and resolves it against this {@code S3Path} in
   * exactly the manner specified by the {@link #resolve(Path) resolve} method.
   *
  ...
  /**
   * Resolves the given path against this path's {@link #getParent parent} path. This is useful
   * where a file name needs to be <i>replaced</i> with another file name. For example, suppose that
   * the name separator is "{@code /}" and a path represents "{@code dir1/dir2/foo}", then invoking
   * this method with the {@code Path} "{@code bar}" will result in the {@code
   * Path} "{@code dir1/dir2/bar}". If this path does not have a parent path, or {@code other} is
   * {@link #isAbsolute() absolute}, then this method returns {@code other}. If {@code other} is an
   * empty path then this method returns this path's parent, or where this path doesn't have a
   * parent, the empty path.
   *
  ...
  /**
   * Converts a given path string to a {@code Path} and resolves it against this path's
   * {@link #getParent parent} path in exactly the manner specified by the
   ...
   * this path is {@code "/a/b"} and the given path is {@code "/a/b/c/d"} then the resulting
   * relative path would be {@code "c/d"}. Where this path and the given path do not have a
   * {@link #getRoot root} component, then a relative path can be constructed. A relative path
   * cannot be constructed if only one of the paths have a root component. Where both paths have a
   * root component then it is implementation dependent if a relative path can be constructed. If
   * this path and the given path are {@link #equals equal} then an <i>empty path</i> is returned.
   *
  ...
  public S3Path relativize(Path other) {
    S3Path otherPath = requireNonNull(MorePaths.toS3Path(other));
    ...
    if (differenceCount < otherNameCount) {
      return getRelativePathFromDifference(otherPath, otherNameCount, differenceCount,
          parentDirCount);
    }
  ...
  private S3Path getRelativePathFromDifference(S3Path otherPath, int otherNameCount,
      int differenceCount, int parentDirCount) {
    ...
    // we need to pop up some directories (each of which needs three characters ../) then append the
    // remaining sub-path
    int relativePathSize =
        parentDirCount * 3 + remainingSubPath.pathRepresentation.toString().length();
   ...
   *
   * <p>
   * This method constructs an absolute and normalized {@link URI} with a {@link URI#getScheme()
   * scheme} equal to the URI scheme that identifies the provider (s3). Please note that the
   * returned URI is a well formed URI, which means all special characters (e.g. blanks, %, ?, etc.)
   * are encoded. This may result in a different string from the real path (object key), which
   * instead allows those characters.
   * <p>
   * For instance, the S3 URI "s3://mybucket/with space and %" is a valid S3 object key, which must
   * be encoded when creating a Path and that will be encoded when creating a URI. E.g.:
   *
   * <pre>
   * {@code
   ...
   * @throws IOError if an I/O error occurs obtaining the absolute path, or where a file system is
   *         constructed to access the contents of a file as a file system, and the URI of the
   *         enclosing file system cannot be obtained
   * @throws SecurityException In the case of the default provider, and a security manager is
   *         installed, the {@link #toAbsolutePath toAbsolutePath} method throws a security
   *         exception.
   */
  ...
  public URI toUri() {
    ...
    elements.forEachRemaining((e) -> {
      ...
      uri.append(PATH_SEPARATOR).append(UrlEncoding.urlencode(name));
    });
    if (isDirectory()) {
   ...
   * path. Otherwise, this method resolves the path by resolving the path against the root (the top
   * level of the bucket). The resulting path may contain redundancies and may point to a
  ...
  public S3Path toAbsolutePath() {
    ...
    return new S3Path(fileSystem,
        PosixLikePathRepresentation.of(PATH_SEPARATOR, pathRepresentation.toString()));
  }
   ...
   *
   * <p>
   * If this path is relative then its absolute path is first obtained, as if by invoking the
   * {@link #toAbsolutePath toAbsolutePath} method. When deriving the <em>real path</em>, and a
   * "{@code ..}" (or equivalent) is preceded by a non-"{@code ..}" name then an implementation will
   * cause both names to be removed.
   *
   * @param options options indicating how symbolic links are handled. S3 has no links so this will
   *        be ignored.
  ...
  public WatchKey register(WatchService watcher, WatchEvent.Kind<?>[] events,
      WatchEvent.Modifier... modifiers) throws UnsupportedOperationException {
    throw new UnsupportedOperationException(
   ...
   *
   * <p>
   * The first element returned by the iterator represents the name element that is closest to the
   * root in the directory hierarchy, the second element is the next closest, and so on. The last
   * element returned is the name of the file or directory denoted by this path. The {@link #getRoot
   * root} component, if present, is not returned by the iterator.
   *
   * @return an iterator over the name elements of this path.
   */
  @Override
  public Iterator<Path> iterator() {
    return new S3PathIterator(pathRepresentation.elements().iterator(),
        pathRepresentation.isAbsolute(), pathRepresentation.hasTrailingSeparator());
  }

  /**
   * Compares two abstract paths lexicographically. The ordering defined by this method is provider
   * specific, and in the case of the default provider, platform specific. This method does not
   * access the file system and neither file is required to exist.
   *
   * <p>
   ...
   * @return zero if the argument is {@link #equals equal} to this path, a value less than zero if
   *         this path is lexicographically less than the argument, or a value greater than zero if
   *         this path is lexicographically greater than the argument
   * @throws ClassCastException if the paths are associated with different providers
   */
  @Override
  public int compareTo(Path other) {
    S3Path o = requireNonNull(MorePaths.toS3Path(other));
    if (o.fileSystem != this.fileSystem) {
      throw new ClassCastException("compared S3 paths must be from the same bucket");
    }
    return this.toRealPath(NOFOLLOW_LINKS).toString()
        .compareTo(o.toRealPath(NOFOLLOW_LINKS).toString());
  }

  /**
   * Tests this path for equality with the given object.
   * <p>
   * {@code true} if {@code other} is also an {@code S3Path} from the same bucket and the two paths
   * have the same real path.
   *
   * @param other the object to which this object is to be compared
   * @return {@code true} if, and only if, the given object is a {@code Path} that is identical to
   *         this {@code Path}
   */
  @Override
   ...
   *
   * <p>
   * The hash code is based upon the components of the path, and satisfies the general contract of
   * the {@link Object#hashCode Object.hashCode} method.
   *
  ...
  /**
   * The key of the object for S3. Essentially the "real path" with the "/" prefix and bucket name
   * removed.
   *
   * @return the key
   */
  ...
  private final class S3PathIterator implements Iterator<Path> {
    ...
    private S3PathIterator(Iterator<String> delegate, boolean isAbsolute,
