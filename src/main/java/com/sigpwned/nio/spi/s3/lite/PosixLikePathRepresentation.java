...
class PosixLikePathRepresentation {
   ...
   * <p>
   * The current implementation is permissive and assumes most characters are legal in the path
   * however care should be taken when using characters which although legal in S3 can cause
   * encoding problems or may not be legal in Posix paths.
   * </p>
   * .
   * <p>
   * When {@code first} begins with "/" it is assumed to be absolute and the root if first is only
   * "/" and {@code more} is empty or null
   * </p>
   * .
   * <p>
   * When {@code first} is the only element and it ends with "/" then a directory is assumed.
   * Likewise if {@code more} is present and the last item of {@code more} ends with "/" then a
   ...
   * @return the representation of the path constructed from the arguments with non-redundant
   *         separators.
   */
  static PosixLikePathRepresentation of(String first, String... more) {
    if ((first == null || first.trim().isEmpty()) && !(more == null || more.length == 0)) {
      throw new IllegalArgumentException(
          "The first element of the path may not be null or empty when more exists");
    }
    ...
    Deque<String> allParts = new LinkedList<String>();
    allParts.add(first);
    ...
    if (allParts.peekLast() == null) {
      throw new RuntimeException(
          "the last element of the path representation is unexpectedly null");
    }
  ...
  private static String partsToPathString(Deque<String> allParts, boolean endsWithSeparator,
      boolean startsWithSeparator) {
    String path = allParts.stream().flatMap(part -> Arrays.stream(part.split("/+")))
        .filter(p -> !p.isEmpty()).collect(Collectors.joining(PATH_SEPARATOR));
  ...
  private static List<String> collectMore(String[] more) {
    if (more != null && more.length != 0) {
      return Arrays.stream(more).filter(Objects::nonNull).filter(p -> !p.isEmpty())
  ...
  /**
   * Is the path resolvable without any further information (e.g. not relative to some other
   * location).
   *
